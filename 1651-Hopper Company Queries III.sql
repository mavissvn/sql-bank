...
cte2 AS (
    ...
    SUM(ride_distance) AS month_distance,  
    SUM(ride_duration) AS month_duration
    FROM Rides a
...
SELECT a.month, IFNULL(ROUND(SUM(month_distance)/3, 2), 0) AS average_ride_distance,
IFNULL(ROUND(SUM(month_duration)/3, 2), 0) AS average_ride_duration
FROM cte1 a
LEFT JOIN cte2 b
ON b.month - a.month BETWEEN 0 AND 2
GROUP BY a.month;